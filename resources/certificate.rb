...
function Add-AccessRule
{
  ...
  if ($Certificate -eq $null) { 
	  throw 'no certificate exists for: $Certificate' 
  }
  
  $cryptoTriage = [Contoso.CryptoTriage]::TriageAcquireKeyHandle($Certificate)
  
  EOH
    new_resource.private_key_acl.each do |name|
      set_acl_script << "$cryptoTriage.Acl = $cryptoTriage.Acl | Add-AccessRule -UserAccount '#{name}' -FileSystemRights ReadAndExecute -AccessControlType Allow\n"
    end
    set_acl_script
  end
end
